...
public class SetSenderPhone extends Conditions {
    public SetSenderPhone(WebDriver driver) {
        this.driver = driver;
        PageFactory.initElements(driver, this);
    }

    public void setSenderPhone(String phoneNumber) {
        System.out.println("Очікую поле номеру відправника...");
        quickWaitForElementPresent(phoneNumberInForm);
        if (phoneNumberInForm.isDisplayed()) {
            System.out.println();
            System.out.print("Вводимо номер телефону відправника...");
...
//Перевіяємо чи треба чекати таблицю контактів
                if (!listOfContactsIsPresent(counterpartyTypeChooseForm)) {
                    System.out.println();
                    System.out.println("Список контактних осіб відправника ВІДСУТНІЙ... продовжую...");
                } else if (listOfContactsIsPresent(counterpartyTypeChooseForm)) {
                    System.out.println();
                    System.out.println("Список контактних осіб відправника ПРИСУТНІЙ... обираю контактну особу...");
                    if (counterpartyTypeChooseForm.isDisplayed()) {
                        quickWaitForElementPresent(listOfContacts);
                        if (listOfContacts.isDisplayed()) {
                            System.out.println();
                            System.out.println("Присутній список вибору контактної особи відправника...");
                        } else {
                            System.out.println();
                            System.out.println("Список контактних осіб відправника ВІДСУТНІЙ... продовжую...");
                        }
